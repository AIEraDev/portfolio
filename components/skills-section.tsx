...
const skillCategories = {
  Frontend: {
    ...
    skills: [
      ...
      { name: "Next.js", level: 88, icon: "▲", color: "from-gray-800 to-gray-600" },
      ...
      { name: "Vanilla JS", level: 100, icon: "🟨", color: "from-yellow-500 to-orange-500" },
  ...
  Backend: {
    ...
    skills: [
      ...
      { name: "Express.js", level: 90, icon: "🚀", color: "from-gray-700 to-gray-900" },
      { name: "Deno", level: 75, icon: "🦕", color: "from-purple-500 to-indigo-500" },
      ...
      { name: "Prisma ORM", level: 82, icon: "🔷", color: "from-indigo-500 to-purple-500" },
      { name: "REST APIs", level: 90, icon: "🔗", color: "from-teal-500 to-cyan-500" },
    ],
  },
  "AI & Tools": {
    icon: Terminal,
    skills: [
      { name: "OpenAI API", level: 85, icon: "🤖", color: "from-purple-600 to-pink-600" },
      { name: "Git", level: 88, icon: "📝", color: "from-orange-500 to-red-500" },
      { name: "Docker", level: 75, icon: "🐳", color: "from-blue-500 to-blue-700" },
      { name: "AWS", level: 70, icon: "☁️", color: "from-yellow-500 to-orange-600" },
      { name: "Vercel", level: 90, icon: "▲", color: "from-gray-800 to-black" },
      { name: "TensorFlow", level: 65, icon: "🧠", color: "from-orange-600 to-red-600" },
      { name: "Parcel", level: 88, icon: "📦", color: "from-brown-500 to-amber-600" },
    ],
  },
}
    ...
    <section id="skills" className="mb-20">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {skillCategories[activeCategory as keyof typeof skillCategories].skills.map((skill, index) => (
          <div
            key={skill.name}
            className="group p-4 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg hover:bg-white dark:hover:bg-gray-800 hover:shadow-md transition-all duration-300 hover:-translate-y-1 animate-fade-in border border-gray-200/50 dark:border-gray-700/50"
            style={{ animationDelay: `${index * 100}ms` }}
          >
            <div className="flex items-center gap-3 mb-3">
              <span className="text-2xl group-hover:scale-110 transition-transform duration-300">{skill.icon}</span>
              <span className="font-medium text-gray-900 dark:text-white group-hover:bg-gradient-to-r group-hover:from-purple-600 group-hover:to-pink-600 group-hover:bg-clip-text group-hover:text-transparent transition-all duration-300">
                {skill.name}
              </span>
            </div>
